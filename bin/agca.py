...
def main(argv):
    ...
    parser = argparse.ArgumentParser(description="AlternatingGeneticCodeAnnotator\nSegments contigs based on genetic "
                                                 ...
                                                 "with(-out) stop codon reassignment", prog="AGCA")
    parser.add_argument('-i', '--path_to_genome', help="Path to input file, FASTA format",
    ...
    parser.add_argument('-w', '--window_size', help="Window size in bp applied to search for isoform. Only relevant "
                                                    "for isoform prediction. [5000]",
                        default=5000, dest='window_size', required=False, type=int)
    parser.add_argument('-st', '--stride', help='Step size in bp, with which window will be moved along sequence. Only '
    ...
    parser.add_argument('-v', '--verbose', help='verbose', required=False, dest='verbose', action='store_true',
                        default=False)
