# AlternatingGeneticCodeAnnotator (AGCA)

### Table of Contents

- [Description of ACGA](#description-of-acga)
   - [Prediction of (alternating) genetic code(s)](#prediction-of-alternating-genetic-codes)
...
The usage of `multiprocess_agca.py` is exactly the same as for `agca.py`, expect it that takes a text file containing paths to fasta file (one per line) specified with `--genomes`. Additionally, the number of CPUs (`--threads`) and the batch size (`--batch_size`) can be specified.

### Example usage
An example run of AlternatingGeneticCodeAnnotator on Phage2<sup>1</sup> with default settings
<pre><code>agca.py -i ./examples/phage2.fasta -r --isoforms -v -m ./plots -p mgm_results/ -o results/
</pre></code>
...
### Citation
If you use the AGCA, please cite:
