...
def parse_agca_results(path_results, target_genomes, alternative_gcode):
    """
    Function to parse gene prediction and *.tab file generated by AGCA
    ...
    :return: pd.DataFrame, pd.DataFrame, AGCA results, gene annotations
    """
    results = []
    predictions = []
    for f in target_genomes:
...
def get_trna_positions(trnas, switch_points, predictions, alternative_genetic_code, target_genomes, path_genomes, output_dir):
    """
    ...
    :param switch_points: pd.DataFrame, AGCA segementation results
    :param predictions: pd.DataFrame, gene annotation rendered by AGCA
    ...
    for con, tg in zip(contigs, target_genomes):
        ...
        if switch_points[switch_points.contig == con][column_order].values[0] == scenario_1:
            ...
            if mode_strand == '+':
                ...
                rooted_seq = seq.seq[sp1:sp2].reverse_complement() + seq.seq[sp2:].reverse_complement() + seq.seq[:sp1].reverse_complement()
            # update tRNA position in rooted seq
            if mode_strand == '-':
                mode_strand = '+'
            ...
            if mode_strand == '+':
                 ...
                 rooted_seq = seq.seq[sp2:].reverse_complement() + seq.seq[:sp1].reverse_complement() + seq.seq[sp1:sp2].reverse_complement()
            # update tRNA position in rooted seq
            if mode_strand == '-':
                mode_strand = '+'
                current_trnas.strand = np.where(current_trnas.strand.values == 0, 1, 0)
    ...
    positions_trnas.loc[positions_trnas.trna_start < 0, 'trna_start'] += positions_trnas.loc[positions_trnas.trna_start < 0, 'contig_size']
    positions_trnas.loc[positions_trnas.trna_start > positions_trnas.contig_size, 'trna_start'] -= positions_trnas.loc[positions_trnas.trna_start > positions_trnas.contig_size, 'contig_size']
    positions_trnas.loc[positions_trnas.trna_end < 0, 'trna_end'] += positions_trnas.loc[positions_trnas.trna_end < 0, 'contig_size']
    positions_trnas.loc[positions_trnas.trna_end > positions_trnas.contig_size, 'trna_end'] -= positions_trnas.loc[positions_trnas.trna_end > positions_trnas.contig_size, 'contig_size']
...
def main(argv):
    parser = argparse.ArgumentParser(description="Visualizes positions of predicted Sup-tRNAs relative to switch "
                                                 ...
                                                 "code information")
    parser.add_argument('--path_agca_results', help='Directory of AGCA results', required=True)
    ...
    switch_points, predictions = parse_agca_results(args.path_agca_results, target_genomes, args.alternative_gcode)
    positions_trnas = get_trna_positions(sup_trnas, switch_points, predictions, args.alternative_gcode, target_genomes, args.path_to_genomes, args.output_dir)
