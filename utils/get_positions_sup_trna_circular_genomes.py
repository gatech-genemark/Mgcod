...
def parse_mgcod_results(path_results, target_genomes, alternative_gcode):
    ...
    :param alternative_gcode: str, indicating reassigned stop codon
...
def get_trna_positions(trnas, switch_points, predictions, alternative_genetic_code, target_genomes, path_genomes,
    ...
    :param alternative_genetic_code: str, stop codon that is reassigned
    ...
    if alternative_genetic_code == 'opal':
        alternative_genetic_code = 4
        trnas = trnas[trnas.Codon == 'TCA']
    elif alternative_genetic_code == 'amber':
        alternative_genetic_code = 15
        trnas = trnas[trnas.Codon == 'CTA']
    elif alternative_genetic_code == 'ochre':
        alternative_genetic_code = 101
...
def plot_distance_from_switch(positions_trnas, alternative_genetic_code, output_dir):
    ...
    ax.annotate('Code 11 on positive protein-encoding strand', xy=(1.05,
                ...
                fontsize=6)
    ax.annotate('{} recoding on negative protein-encoding strand'.format(alternative_genetic_code),
                xy=(0.05, positions_trnas.loc[:, 'contig'].unique().shape[0] + 5), fontsize=6)
    ...
    labels = ['tRNA gene on positive PES',
              ...
              'Code switch']
    ncol = 3
    ax.legend(handles, labels, bbox_to_anchor=(0.5, -.01), loc='upper center', ncol=ncol, fontsize=7, handlelength=1.5)
    pos_strand = 0
    neg_strand = 0
